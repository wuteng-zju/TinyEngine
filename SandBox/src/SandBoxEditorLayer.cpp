...
void SandBoxEditorLayer::OnUpdate(const TimeStep& timeStep)
{
	...
	TINY_ENGINE_ASSERT(spFrameBuffer, "FrameBuffer is null in Edit Layer");
...
{
	auto sFilePath = PlatformUtils::OpenFile("TinyEngine Scene (*.scene)\0*.scene\0");
	if (!sFilePath.empty())
	{
		OpenScene(sFilePath);
	}
...
{
	auto sFilePath = PlatformUtils::SaveFile("TinyEngine Scene (*.scene)\0*.scene\0");
	if (!sFilePath.empty())
	{
...
void SandBoxEditorLayer::OnSceneStop()
{
	switch (m_eSceneState)
	{
		case TinyEngine::SceneState::Play:
		{
			m_spActiveScene->OnRuntimeStop();
			break;
		}
		case TinyEngine::SceneState::Simulate:
		{
...
void SandBoxEditorLayer::OnOverlayRender()
{
	...
	switch (m_eSceneState)
	{
		...
		case TinyEngine::SceneState::Edit:
		{
